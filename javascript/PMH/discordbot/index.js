...
bwt.on('message', (msg) => {
  ...
  if (msg.channel.id === '607554417524473875' || !msg.guild) {
    ...
    if (msg.content.endsWith('#욕설로추가')) {
      ...
      superagent.get('http://localhost:8080/add/badword/' + query).then((res) => {
      ...
      superagent.get('http://localhost:8080/add/fineword/' + query).then((res) => {
        msg.channel.send('패치완료!') })
    } else if (msg.content.endsWith('#detail') || msg.content.endsWith('#더보기')) {
      let query = encodeURI(msg.content)
      superagent.get('http://localhost:8080/check/' + query).then((res) => {
